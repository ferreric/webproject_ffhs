...
const correctPassword = (pw) => {
    ...
    let max = eval(minMax[1].toString().split(' ')[0])
    let letter = testStringSplit[1].toString().split(':')[0]
    let pwd = testStringSplit[2]
    if (pwd.split(letter).length-1 >= min && pwd.split(letter).length-1 <= max) return true
    else return false
}
let correct1s = 0
const countCorrectPws = (list) => {
    let pwArray = list.split('\n')
    pwArray.forEach(pw => {
        if (correctPassword(pw)) correct1s++
    })
    return correct1s
}
console.log(countCorrectPws(pwList))

const textFeld = document.getElementById('pwcheck-field')
const button = document.getElementById('submit')
const response = document.getElementById('response')

button.addEventListener('click', () => {
    let passwords = textFeld.value;
    correct1s = 0
    countCorrectPws(passwords)
    response.innerText = correct1s
})